...
\title{NRC: a decentralized platform for Human-Robot Communication}
...
\author{
  ...
  \thanks{NEO Robotics is the name of our team.
      ...
      to improve robots' capabilities when working with humans.}
  \\
  Singapore\\
...
and (ii) take actions to achieve its own objectives.
Consider the autonomous driving car as an example, in order to be safe
and efficient, the robot car needs to 
...
Compared with the onboard sensors, NRC can provides much more
information to the robot for it to make better decisions. For
example, the robot will be able to know the position of the
human when they are separated by a wall in the middle, which
can not be achieved by the onboard sensors.
...
Note that NRC is general and not limit to autonomous driving. It can
be potentially be applied to any robots that share their workspace
